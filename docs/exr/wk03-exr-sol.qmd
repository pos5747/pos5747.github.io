...
*Hint:* For the bootstrap, repeatedly simulate $y^{*(b)}_1,\dots,y^{*(b)}_N \stackrel{iid}{\sim} \text{Bernoulli}(\hat \pi)$, recompute $\hat \pi^{*(b)} = \operatorname{avg}(y^{*(b)})$, and then take the empirical $2.5^\text{th}$ and $97.5^\text{th}$ percentiles of $\{\hat \pi^{*(b)}\}$.
